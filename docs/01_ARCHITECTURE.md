...
serialization/deserialization but wrapped in a thin `prow.stix` module so it
can be replaced — the upstream library has been functionally inactive in
recent releases and we may need to maintain a soft fork.
...
**Critical property:** connectors run as `asyncio` tasks in the prow process
in monolith mode, or as separate worker pods in split mode. Same code path.
The `ConnectorContext` abstracts the runtime.
...
6. **STIX Python library is functionally inactive.** Wrap it; plan for soft
